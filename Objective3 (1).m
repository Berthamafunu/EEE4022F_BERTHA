...
N=1.0334;     %From Objective function 2
Lp=0.089618;    %From Objective function 2
...
Icd=((M.*Rr.*Vp)./(Wp.*Wpr))+(M.*Lr.*Lp.*Ip)-(M.*Rr.*Rp.*Ip./(Wpr.*Wp))-(N*Ip);
Icq=((M.*Lr.*Vp)./Wp)-((M.*Rr.*Lp.*Ip)./Wpr)-(M.*Lr.*Rp.*Ip./(Wp));

dataNumber=numel(Wpr);
% number of variables
variableNum=1;
...
%     S=((((120*f)/2)-Wr)/((120*f)/2));
    ...
    Vc_c(1,k)=((Rc.*Icd(1,k))-(Wc(1,k).*A(1).*Icq(1,k))+((Wc(1,k).*Mcr.*(Vp(1,k)-(Rp.*Ip(1,k))))./(Wp(1,k).*Mpr))) + (((Rc.*Icq(1,k))+(Wc(1,k).*A(1).*Icd(1,k))-(Mcr.*Lp.*Wc(1,k).*Ip(1,k))).*1i);
...
%end
disp(['Lc = ' num2str(swarm_BestPosition.variable(1))])
